...
# Directories
WORKDIR=$(pwd)
RAWDATA_DIR="$WORKDIR/data/01.RawReads"
FASTQC_DIR="$WORKDIR/data/QC/01.FastQC_MultiQC"
DONE_DIR="$FASTQC_DIR/.done"

# Create output directories
mkdir -p "$FASTQC_DIR" "$DONE_DIR"

# Sanity check: warn if the array size does not match the number of samples
N_SAMPLES=$(find "$RAWDATA_DIR" -mindepth 1 -maxdepth 1 -type d | wc -l)
